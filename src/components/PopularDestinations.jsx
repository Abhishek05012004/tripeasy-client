...
function scroll(ref, direction) {
  ...
  const setAutoScrollEnabled = ref === window.domesticRef ? window.setIsPausedDomestic : window.setIsPausedInternational
  setAutoScrollEnabled(false)
  const cardWidth = 270
...
function checkScrollPosition(ref) {
  if (!ref || !ref.current) return
  const { scrollLeft, scrollWidth, clientWidth } = ref.current

  if (ref === window.domesticRef) {
    window.setCanScrollLeftDomestic(scrollLeft > 10)
    window.setCanScrollRightDomestic(scrollLeft + clientWidth < scrollWidth - 10)
    if (scrollLeft + clientWidth >= scrollWidth - 10) {
      window.setReachedEndDomestic(true)
      window.setScrollDirectionDomestic("left")
    } else if (scrollLeft <= 10 && window.scrollDirectionDomestic === "left") {
      window.setReachedEndDomestic(false)
      window.setScrollDirectionDomestic("right")
    }
  } else if (ref === window.internationalRef) {
    window.setCanScrollLeftInternational(scrollLeft > 10)
    window.setCanScrollRightInternational(scrollLeft + clientWidth < scrollWidth - 10)
    if (scrollLeft + clientWidth >= scrollWidth - 10) {
      window.setReachedEndInternational(true)
      window.setScrollDirectionInternational("left")
    } else if (scrollLeft <= 10 && window.scrollDirectionInternational === "left") {
      window.setReachedEndInternational(false)
      window.setScrollDirectionInternational("right")
    }
  }
}
